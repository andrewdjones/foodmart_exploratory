...
def getChildren(hier,nm):
    '''
    For name n at hierarchy lev = [family,department,category,product_class_id,brand].index(hier)
    Returns list of children in level (l+1)
    '''
...
def revByGroup(hier):
    ...
    for p_id,p_class in product_id_lookups.items():
        ...
        if hier=='product_category':
            ...
            raise NameError('Mode must be category, department, or family')
    ...
    hierRev = collections.Counter()
    hierCost = collections.Counter()
...
def revByTwoGroups(g1,g2):
    ...
    for id,sales,cost in trans_data:
        #print product_class_lookups[1][id]
...
def revenueByQuarter(mode = 'categories',stores = 'all',track='store_sales'):
    ...
    if stores is an int, pulls quarterly revenue for that particular store_id
    if stores != 'all' (and also isn't an int), tallies quarterly revenue per store,
    ...
    if mode=='categories':
        ...
        for p_id,p_class in product_id_lookups.items():
            product_class_lookups[p_id] = product_cat_lookups[p_class][0]
    elif mode=='brands':#under construction
        ...
        if stores == 'all' or type(stores) == int:
            ...
            if track == 'store_sales,store_cost':
                for id,sales,cost in trans_data:
                    ...
                    quarterly_trans[j+1][product_class_lookups[id]] += sales - cost 
...
def quarterlyRevByStore(viz=True,t='quarterly',track='sales'):
    '''
    Pulls quarterly or annual data by store from revenueByQuarter()
...
def visualizeRevenue(quarter, topN):
    '''
    Brand revenue plot
...
def profitWithinGroup(hier,nm):
    ...
    hier should be in tree (and not 'brand')
    '''
    tree = ['product_family','product_department','product_category','product_class_id','brand']
    ...
    roi = [(y+y2)/y2 for y,y2 in zip(yvals,y2vals)]
    yy2x = zip(roi,yvals,y2vals,xlabs)
    yy2x.sort(reverse=True)
    #plot the stacked bars, located by index ind
    ind = range(len(xlabs))    
    plt.bar(ind,[y for r,y,y2,x in yy2x],width,color='r',bottom=[y2 for r,y,y2,x in yy2x],label='Profit')
    plt.bar(ind,[y2 for r,y,y2,x in yy2x],width,color='b',label='Cost')
    #pyplot markup
...
def profitByBrand(hier,nm):
    ...
    tree = ['product_class','product_category','product_department','product_family']
    lvl = tree.index(hier)
    for product_id,store_sales,store_cost in trans:
        if lvl == 0:
            if product_cat_lookups[brandsByProduct_id[product_id][0]] == nm:
    ...
    plt.bar(ind,[y2 for r,y,y2,x in yy2x],width,color='b',label='Cost')
    #pyplot markup
    plt.suptitle('Annual revenue for '+hier+' '+nm+' by brand')
    plt.xticks([x + width/2 for x in range(len(xlabs))],[x for r,y,y2,x in yy2x],rotation=45)
...
profitByBrand('product_family', 'Non-Consumable')    
#profitWithinGroup('product_category', '"Snack Foods"')
#getChildren('product_family', '"Drink"')
#quarterlyRevByStore(t='quarterly',track='profit')
#visualizeRevenue('average', 20, mode = 'brands')